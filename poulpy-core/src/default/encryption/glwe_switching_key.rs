    ...
    for<'s> ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
{
        ...
        S2: GLWESecretToBackendRef<BE> + GetDistribution + GLWEInfos,
    {
        ...
        assert!(
            scratch.available() >= GLWESwitchingKeyEncryptSkDefault::glwe_switching_key_encrypt_sk_tmp_bytes(self, res),
            "scratch.available(): {} < GLWESwitchingKeyEncryptSk::glwe_switching_key_encrypt_sk_tmp_bytes: {}",
            scratch.available(),
            GLWESwitchingKeyEncryptSkDefault::glwe_switching_key_encrypt_sk_tmp_bytes(self, res)
        );
