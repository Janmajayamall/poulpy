        ...
        BE: 's,
    {
        ...
        for row in 0..res.dnum().into() {
            for col in 0..res.rank_in().into() {
                let mut res_at = gglwe_at_backend_mut_from_mut::<BE>(&mut res, row, col);
                let mut res_view = &mut res_at;
                let a_at = gglwe_at_backend_ref_from_ref::<BE>(&a, row, col);
                let a_view = &a_at;
                self.glwe_keyswitch_default(&mut res_view, &a_view, b, &mut scratch.borrow());
            }
        }
    }
        ...
        BE: 's,
    {
        ...
        for row in 0..res.dnum().into() {
            for col in 0..res.rank_in().into() {
                let mut res_at = gglwe_at_backend_mut_from_mut::<BE>(&mut res, row, col);
                let mut res_view = &mut res_at;
                self.glwe_keyswitch_assign_default(&mut res_view, a, &mut scratch.borrow());
            }
        }
    }
}
