    ...
    Self: ModuleN + GLWEKeyswitchDefault<BE> + GGSWExpandRowsDefault<BE>,
{
        ...
        ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
    {
        ...
        for row in 0..res.dnum().into() {
            let mut res_at = ggsw_at_backend_mut_from_mut::<BE>(&mut res, row, 0);
            let mut res_view = &mut res_at;
            self.glwe_keyswitch_assign_default(&mut res_view, key, &mut scratch.borrow());
        }
        ...
        ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
    {
        ...
        for row in 0..a.dnum().into() {
            let mut res_at = ggsw_at_backend_mut_from_mut::<BE>(&mut res, row, 0);
            let mut res_view = &mut res_at;
            let a_at = ggsw_at_backend_ref_from_ref::<BE>(&a, row, 0);
            let a_view = &a_at;
            self.glwe_keyswitch_default(&mut res_view, &a_view, key, &mut scratch.borrow());
        }
