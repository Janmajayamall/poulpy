...
#[doc(hidden)]
pub(crate) trait GGSWAutomorphismDefault<BE: Backend>
where
    Self: GLWEAutomorphismDefault<BE> + GGSWExpandRowsDefault<BE>,
{
        ...
        ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
    {
        ...
        for row in 0..res_backend.dnum().as_usize() {
            ...
            let a_at = ggsw_at_backend_ref_from_ref::<BE>(&a_backend, row, 0);
            let a_view = &a_at;
            self.glwe_automorphism_default(&mut res_view, &a_view, key, &mut scratch.borrow());
        }
        ...
        ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
    {
        ...
        for row in 0..res_backend.dnum().as_usize() {
            let mut res_at = ggsw_at_backend_mut_from_mut::<BE>(&mut res_backend, row, 0);
            let mut res_view = &mut res_at;
            self.glwe_automorphism_assign_default(&mut res_view, key, &mut scratch.borrow());
        }
