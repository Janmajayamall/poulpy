use poulpy_hal::{
    ...
    layouts::{
        Backend, Module, ScratchArena, VecZnx, VecZnxBackendRef, VecZnxBigToBackendRef, VecZnxDftBackendRef,
...
pub(crate) trait GGSWFromGGLWEDefault<BE: Backend>: GGSWExpandRowsDefault<BE> + GLWECopyDefault<BE>
where
        ...
        for<'a> ScratchArena<'a, BE>: ScratchArenaTakeCore<'a, BE>,
    {
        ...
        let mut res_backend = res.to_backend_mut();
        {
            ...
            for row in 0..res_backend.dnum().into() {
                let mut res_at = ggsw_at_backend_mut_from_mut::<BE>(&mut res_backend, row, 0);
                let a_at = gglwe_at_backend_ref_from_ref::<BE>(&a_backend, row, 0);
                self.glwe_copy(&mut res_at, &a_at);
            }
        }
...
where
    Self: GGSWExpandRowsDefault<BE> + GLWECopyDefault<BE>,
    for<'s> ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
{
}
