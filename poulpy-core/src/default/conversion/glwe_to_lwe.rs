...
use crate::{
    ...
    layouts::{
        GGLWEInfos, GLWE, GLWEInfos, GLWELayout, GLWEToBackendRef, LWEInfos, LWEToBackendMut, Rank, glwe_backend_ref_from_mut,
    ...
    Self: ModuleN + GLWEKeyswitchDefault<BE> + GLWERotate<BE> + VecZnxCopyRangeBackend<BE> + VecZnxZeroBackend<BE>,
{
        ...
        for<'x> ScratchArena<'x, BE>: ScratchArenaTakeCore<'x, BE>,
    {
        ...
        self.glwe_keyswitch_default(&mut tmp_glwe_rank_1, &a_backend_view, key, &mut scratch_1);
        if a_idx != 0 {
            self.glwe_rotate_assign(-(a_idx as i64), &mut tmp_glwe_rank_1, &mut scratch_1);
        }
