    ...
    Self: ModuleLogN + GaloisElement + GLWEAutomorphism<BE> + GLWEShift<BE> + GLWECopy<BE> + CyclotomicOrder + GLWENormalize<BE>,
{
        ...
        BE: 's,
    {
        ...
        if a.base2k() == atk_layout.base2k() {
            ...
            scratch_1 = scratch_1.apply_mut(|scratch| {
                self.glwe_normalize(&mut tmp, a, scratch);
            });
        }

        {
            scratch_1 = scratch_1.apply_mut(|scratch| {
                trace_assign_internal::<Self, K, H, _, BE>(self, &mut tmp, skip, keys, scratch);
            });
        }
        ...
        if res.base2k() == atk_layout.base2k() {
            ...
            let (mut res_out, scratch_2) = scratch_1.take_glwe_scratch(&res.glwe_layout());
            {
                scratch_2.apply_mut(|scratch| {
                    self.glwe_normalize(&mut res_out, &tmp, scratch);
                });
            }
    ...
    where
        ...
        BE: 's,
    {
        trace_assign_internal::<Self, K, H, _, BE>(self, res, skip, keys, scratch);
    }
}
