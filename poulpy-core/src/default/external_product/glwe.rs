...
use crate::{api::GLWEExternalProductInternal, default::operations::GLWENormalizeDefault};
    ...
    for<'x> ScratchArena<'x, BE>: ScratchArenaTakeCore<'x, BE>,
{
    ...
    {
        let scratch_phase = scratch.borrow();
        ...
        if dsize == 1 {
            ...
            for di in 0..dsize {
                ...
                if di == 0 {
                    let a_dft_ref = a_dft.to_backend_ref();
                    module.vmp_apply_dft_to_dft(res_dft, &a_dft_ref, &ggsw.data, 0, &mut scratch_2.borrow());
                } else {
                    res_dft_tmp.set_size(res_dft.size());
                    let a_dft_ref = a_dft.to_backend_ref();
                    module.vmp_apply_dft_to_dft(&mut res_dft_tmp, &a_dft_ref, &ggsw.data, di, &mut scratch_2.borrow());
    ...
    for<'s> ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
{
    ...
    fn glwe_external_product_assign_default<'s, R, D>(&self, res: &mut R, ggsw: &D, scratch: &mut ScratchArena<'s, BE>)
    where
        ...
        BE: 's,
    {
        ...
        if res_base2k != ggsw_base2k {
            scratch.scope(|scratch_phase| {
                ...
                self.glwe_normalize(&mut res_conv, res, &mut scratch_2.borrow());
                self.glwe_external_product_dft(&mut res_dft, &res_conv, ggsw, &mut scratch_2);
            });
        } else {
            self.glwe_external_product_dft(&mut res_dft, res, ggsw, &mut scratch.borrow());
        }
    ...
    fn glwe_external_product_default<'s, R, A, G>(&self, res: &mut R, a: &A, ggsw: &G, scratch: &mut ScratchArena<'s, BE>)
    where
        R: GLWEToBackendMut<BE> + GLWEInfos,
        ...
        BE: 's,
    {
        ...
        if a_base2k != ggsw_base2k {
            scratch.scope(|scratch_phase| {
                ...
                self.glwe_external_product_dft(&mut res_dft, &a_conv, ggsw, &mut scratch_2);
            });
        } else {
            self.glwe_external_product_dft(&mut res_dft, a, ggsw, &mut scratch.borrow());
        }
