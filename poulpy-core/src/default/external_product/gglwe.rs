...
#[doc(hidden)]
pub(crate) trait GGLWEExternalProductDefault<BE: Backend>
where
        ...
        let res_rank_in: usize = res.rank_in().into();
        {
            ...
            for row in 0..min_dnum {
                for col in 0..res_rank_in {
                    let mut res_at = crate::layouts::gglwe_at_backend_mut_from_mut::<BE>(&mut res, row, col);
                    let mut res_view = &mut res_at;
                    let a_at = crate::layouts::gglwe_at_backend_ref_from_ref::<BE>(&a, row, col);
                    let a_view = &a_at;
                    self.glwe_external_product_default(&mut res_view, &a_view, b, &mut scratch.borrow());
                }
            }
        }
        ...
        BE: 's,
    {
        ...
        for row in 0..res_dnum {
            for col in 0..res_rank_in {
                let mut res_at = crate::layouts::gglwe_at_backend_mut_from_mut::<BE>(&mut res, row, col);
                let mut res_view = &mut res_at;
                self.glwe_external_product_assign_default(&mut res_view, a, &mut scratch.borrow());
            }
        }
    }
}
