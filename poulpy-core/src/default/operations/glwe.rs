...
pub use crate::api::{GLWEMulConst, GLWEMulPlain, GLWENormalize, GLWERotate, GLWEShift, GLWETensoring};
use crate::{
    ...
    layouts::{
        ...
        LWEInfos, glwe_backend_mut_from_mut, glwe_backend_ref_from_ref, prepared::GLWETensorKeyPreparedToBackendRef,
    },
};
...
impl<BE: Backend> GLWEAddDefault<BE> for Module<BE> where
...
impl<BE: Backend> GLWESubDefault<BE> for Module<BE> where
    Self: ModuleN
        + VecZnxSubBackend<BE>
        + VecZnxSubAssignBackend<BE>
...
impl<BE: Backend> GLWENegateDefault<BE> for Module<BE> where
...
impl<BE: Backend> GLWECopyDefault<BE> for Module<BE> where Self: ModuleN + VecZnxCopyBackend<BE> + VecZnxZeroBackend<BE> {}
...
pub(crate) fn glwe_normalize_backend_ref<BE, M, R>(
    module: &M,
    res: &mut R,
    ...
    BE: Backend,
    M: ?Sized + ModuleN + VecZnxNormalize<BE> + VecZnxNormalizeTmpBytes,
    R: GLWEToBackendMut<BE>,
    for<'s> ScratchArena<'s, BE>: ScratchArenaTakeCore<'s, BE>,
{
    let mut res = res.to_backend_mut();
    ...
    assert_eq!(res.rank(), a.rank());
    assert!(
        scratch.available() >= module.vec_znx_normalize_tmp_bytes(),
    ...
    Self: ModuleN + VecZnxNormalize<BE> + VecZnxNormalizeAssignBackend<BE> + VecZnxNormalizeTmpBytes,
{
        ...
        ScratchArena<'a, BE>: ScratchArenaTakeCore<'a, BE>,
    {
        ...
        *tmp_slot = Some(tmp);
        ...
        glwe_normalize_backend_ref(self, tmp_ref, &glwe_ref, &mut scratch2);
        ...
        ScratchArena<'a, BE>: ScratchArenaTakeCore<'a, BE>,
    {
        ...
        *tmp_slot = Some(tmp);
        ...
        self.glwe_normalize(tmp_ref, &glwe, &mut scratch2);
    ...
    {
        let a = a.to_backend_ref();
        glwe_normalize_backend_ref(self, res, &a, scratch);
    }
