...
use crate::{CKKSInfos, leveled::api::CKKSSubOpsUnsafe};
...
pub fn test_sub_ct_aligned_unsafe<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    ...
    unsafe {
        ctx.module
            .ckks_sub_into_unsafe(&mut ct_res, &ct1, &ct2, &mut scratch.borrow())
            .unwrap();
    }
...
pub fn test_sub_ct_assign_aligned_unsafe<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    ...
    unsafe {
        ctx.module
            .ckks_sub_assign_unsafe(&mut ct1, &ct2, &mut scratch.borrow())
            .unwrap();
    }
...
pub fn test_sub_pt_vec_znx_into_unsafe<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    ...
    unsafe {
        ctx.module
            .ckks_sub_pt_vec_znx_into_unsafe(&mut ct_res, &ct1, &pt_znx, &mut scratch.borrow())
            .unwrap();
    }
...
pub fn test_sub_pt_const_znx_into_aligned_unsafe<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    ...
    unsafe {
        ctx.module
            .ckks_sub_pt_const_znx_into_unsafe(&mut ct_res, &ct, 0, &cst_znx, 0, &mut scratch.borrow())
            .unwrap();
        ctx.module
            .ckks_sub_pt_const_znx_assign_unsafe(&mut ct_res, ctx.m(), &cst_znx, 1, &mut scratch.borrow())
            .unwrap();
    }
