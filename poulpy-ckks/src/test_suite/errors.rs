...
pub fn test_reallocate_limbs_checked_error<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
...
pub fn test_compact_limbs_copy<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let mut ct = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
...
pub fn test_dot_product_overflow_guard<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let host_module = Module::<HostBytesBackend>::new(Degree(8).as_usize() as u64);
    let mut dst = host_module.ckks_ciphertext_alloc(Base2K(63), TorusPrecision(64));
    dst.meta = ctx.meta();
    let a = host_module.ckks_ciphertext_alloc(Base2K(63), TorusPrecision(64));
    let b = host_module.ckks_ciphertext_alloc(Base2K(63), TorusPrecision(64));
    let a_refs = vec![&a, &a];
    let b_refs = vec![&b, &b];
    let err = ctx
        .module
        .ckks_dot_product_ct(&mut dst, &a_refs, &b_refs, ctx.tsk(), &mut scratch.borrow())
        .unwrap_err()
        .to_string();
    assert!(
