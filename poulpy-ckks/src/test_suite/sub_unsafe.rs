...
//! | [`test_sub_pt_vec_znx_into_unsafe`] | ct - ZNX plaintext, `VecZnxRshSubBackend` |
...
pub fn test_sub_pt_vec_znx_into_unsafe<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let ct1 = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    ...
    let mut ct_res = UnnormalizedCKKSCiphertext::new(ctx.alloc_ct(ctx.max_k()));
    ctx.module
        .ckks_sub_pt_vec_znx_into_unnormalized(&mut ct_res, &ct1, &pt_znx, &mut scratch.borrow())
        .unwrap();
    assert_unary_output_meta("sub_pt_vec_znx_into_unsafe", ct_res.as_inner(), &ct1);
    let ct_res = ct_res.normalize(&ctx.module, &mut scratch.borrow());
    ctx.assert_decrypt_precision(
...
pub fn test_sub_pt_const_znx_into_aligned_unsafe<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    ...
    ctx.module
        .ckks_sub_pt_const_znx_into_unnormalized(&mut ct_res, &ct, 0, &cst_znx, 0, &mut scratch.borrow())
        .unwrap();
    ctx.module
        .ckks_sub_pt_const_znx_assign_unnormalized(&mut ct_res, ctx.m(), &cst_znx, 1, &mut scratch.borrow())
        .unwrap();
    assert_unary_output_meta("sub_pt_const_znx_into_aligned_unsafe", ct_res.as_inner(), &ct);
    let ct_res = ct_res.normalize(&ctx.module, &mut scratch.borrow());
    ctx.assert_decrypt_precision(
