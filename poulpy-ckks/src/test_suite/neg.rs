...
pub fn test_neg_aligned<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) -> Result<()> {
    let mut scratch = ctx.alloc_scratch();
    let ct1 = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let (want_re, want_im) = ctx.want_neg();
    let mut ct_res = ctx.alloc_ct(ctx.max_k());
    ctx.module.ckks_neg_into(&mut ct_res, &ct1, &mut scratch.borrow())?;
    assert_unary_output_meta("neg", &ct_res, &ct1);
    ctx.assert_decrypt_precision("neg", &ct_res, &want_re, &want_im, &mut scratch.borrow());
    Ok(())
}

/// Negation out-of-place.
pub fn test_neg_smaller_output<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) -> Result<()> {
    let mut scratch = ctx.alloc_scratch();
    let ct1 = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let (want_re, want_im) = ctx.want_neg();
...
/// Negation in-place.
pub fn test_neg_assign<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) -> Result<()> {
    let mut scratch = ctx.alloc_scratch();
    ...
    ctx.assert_decrypt_precision("neg_assign", &ct, &want_re, &want_im, &mut scratch.borrow());
    Ok(())
}
