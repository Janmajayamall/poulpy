...
pub fn test_mul_ct_aligned<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    ...
    ctx.module
        .ckks_mul_into(&mut ct_res, &ct1, &ct2, ctx.tsk(), &mut scratch.borrow())
        .unwrap();
    assert_mul_ct_output_meta("mul_ct_aligned", &ct_res, &ct1, &ct2);
    ctx.assert_decrypt_precision("mul_ct_aligned", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}

/// ct × ct, a.log_budget() < b.log_budget() (a rescaled by one limb).
pub fn test_mul_ct_delta_a_lt_b<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let ct1 = ctx.encrypt(
    ...
    ctx.assert_decrypt_precision("mul_ct a_lt_b", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}

/// ct × ct, a.log_budget() > b.log_budget() (b rescaled by one limb).
pub fn test_mul_ct_delta_a_gt_b<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let ct1 = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let ct2 = ctx.encrypt(
        ctx.max_k() - ctx.base2k().as_usize() + 1,
    ...
    ctx.assert_decrypt_precision("mul_ct a_gt_b", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}

/// ct × ct with aligned homomorphic capacity but different log_delta.
pub fn test_mul_ct_delta_log_delta<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
...
pub fn test_mul_ct_smaller_output<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let ct1 = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let ct2 = ctx.encrypt(ctx.max_k(), &ctx.re2, &ctx.im2, &mut scratch.borrow());
    let (want_re, want_im) = ctx.want_mul();
    ...
    ctx.assert_decrypt_precision("mul_ct smaller_output", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}

// ─── ct × ct out-of-place (GLWE<_, CKKS>::mul_assign) ─────────────────────────────────

/// ct × ct multiplication with both inputs at the same log_budget().
pub fn test_mul_ct_assign_aligned<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let mut ct_res = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let ct1 = ctx.encrypt(ctx.max_k(), &ctx.re2, &ctx.im2, &mut scratch.borrow());
    let (want_re, want_im) = ctx.want_mul();
    let ct_res_meta = ct_res.meta();
    ctx.module
        .ckks_mul_assign(&mut ct_res, &ct1, ctx.tsk(), &mut scratch.borrow())
        .unwrap();
...
pub fn test_mul_ct_assign_self_lt<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
...
/// ct-ct in-place, self.log_budget() > a.log_budget() (a is shifted down to align with self).
pub fn test_mul_ct_assign_self_gt<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let mut ct_res = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let ct1 = ctx.encrypt(
        ctx.max_k() - ctx.base2k().as_usize() - 1,
...
pub fn test_square_aligned<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let ct = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let (want_re, want_im) = ctx.want_square();
    let mut ct_res = ctx.alloc_ct(ctx.max_k());
    ctx.module
        .ckks_square_into(&mut ct_res, &ct, ctx.tsk(), &mut scratch.borrow())
        .unwrap();
    assert_mul_ct_output_meta("square_aligned", &ct_res, &ct, &ct);
    ctx.assert_decrypt_precision("square_aligned", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}

/// ct² after the input has already been rescaled by one limb.
pub fn test_square_rescaled_input<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let ct = ctx.encrypt(
        ctx.max_k() - ctx.base2k().as_usize() + 1,
        &ctx.re1,
    ...
    ctx.assert_decrypt_precision("square_rescaled_input", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}

/// ct² into an output buffer with smaller k.
pub fn test_square_smaller_output<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let ct = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let (want_re, want_im) = ctx.want_square();
    let mut ct_res = ctx.alloc_ct(ctx.max_k() - ctx.base2k().as_usize() - 1);
    ...
    ctx.assert_decrypt_precision(" square_smaller_output", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}

// ─── ct² squaring inplace (GLWE<_, CKKS>::square) ───────────────────────────────────────

/// ct² inplace.
pub fn test_square_assign<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let mut ct = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let (want_re, want_im) = ctx.want_square();
    let ct_in_meta = ct.meta();
    ctx.module
        .ckks_square_assign(&mut ct, ctx.tsk(), &mut scratch.borrow())
        .unwrap();
...
pub fn test_mul_pt_const_assign<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let mut ct = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let (const_re_f64, _) = ctx.sample_mul_const();
    let (const_re, const_im) = ctx.quantized_const_pt(const_re_f64, 0.0);
...
pub fn test_mul_pt_const_into_delta_log_delta<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let pt_prec = ctx.meta_pt();
    let (a_re, a_im) = ctx.quantized_vector(TestVector::First, ctx.meta().log_delta);
    let (const_re_f64, _) = ctx.sample_mul_const();
...
pub fn test_mul_ct_explicit_metadata_error<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
    let mut ct1 = ctx.encrypt(ctx.max_k(), &ctx.re1, &ctx.im1, &mut scratch.borrow());
    let mut ct2 = ctx.encrypt(ctx.max_k(), &ctx.re2, &ctx.im2, &mut scratch.borrow());
