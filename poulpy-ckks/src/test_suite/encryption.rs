...
fn extract_src_prec<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) -> CKKSMeta {
...
fn extract_fixture<'a, BE: Backend + 'a, F: TestScalar, E: NegacyclicFFT<F>>(
...
fn assert_decrypt_extract_success<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(
    label: &str,
    ctx: &TestContext<BE, F, E>,
    dst_prec: CKKSMeta,
) where
...
pub fn test_encrypt_decrypt<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let mut scratch = ctx.alloc_scratch();
...
pub fn test_decrypt_extract_same_meta<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    assert_decrypt_extract_success("decrypt_extract_same_meta", ctx, extract_src_prec(ctx));
}

pub fn test_decrypt_extract_truncates_log_budget<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let src_prec = extract_src_prec(ctx);
...
pub fn test_decrypt_extract_rsh_for_smaller_log_delta<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(
    ctx: &TestContext<BE, F, E>,
) {
    let src_prec = extract_src_prec(ctx);
    assert_decrypt_extract_success(
...
pub fn test_decrypt_extract_lsh_for_larger_log_delta<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(
    ctx: &TestContext<BE, F, E>,
) {
    let src_prec = extract_src_prec(ctx);
    assert_decrypt_extract_success(
...
pub fn test_decrypt_extract_output_hom_rem_too_large<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(
    ctx: &TestContext<BE, F, E>,
) {
    let src_prec = extract_src_prec(ctx);
    let mut scratch = ctx.alloc_scratch();
...
pub fn test_decrypt_extract_base2k_mismatch_error<BE: Backend, F: TestScalar, E: NegacyclicFFT<F>>(ctx: &TestContext<BE, F, E>) {
    let src_prec = extract_src_prec(ctx);
    let mut scratch = ctx.alloc_scratch();
    let ct = extract_fixture(ctx, &mut scratch.borrow());
